											...
											<div class="col-xl-12 add_slider_content">
												<form action="" method="post" enctype="multipart/form-data">
													<?php
														...
														$select_sql = "SELECT * FROM `home_page` WHERE id = $slider_id";
														$query = mysqli_query($conn,$select_sql);
														while ($row = mysqli_fetch_assoc($query)) {
															echo '
																	<input type="text" value="'.$row['title'].'" name="slider_title">
																	<textarea style="height:100px;margin-top:25px;" type="text" name="slider_description">'.$row['description'].'</textarea>
																	<input type="file" name="slider_img" value="'.$row['img'].'">
																';
														}
			...
			if (isset($_POST['submit_slider'])) {
				...
				if (in_array($exten, $extension)) {
					...
					if (move_uploaded_file($slider_tmp, $folder_path)) {
						$insert_sql = "UPDATE `home_page` SET `title`= '$slider_title',`description`= '$slider_description',`img`= '$folder_path' WHERE id = $slider_id";
